...
const
  // 아바타 카드(자리 정보 패널): 아바타가 상단을 가로로 꽉 채우고 그 아래에 정보가 쌓인다.
  PANEL_W = 112;
  // 아바타 원본이 정사각형(128x128)이라 폭을 꽉 채우면 높이도 PANEL_W가 된다.
  // 거기에 이름·머니·전적·배지가 들어갈 정보 영역을 더한 값이 카드 높이다.
  PANEL_INFO_H = 86;
  PANEL_H = PANEL_W + PANEL_INFO_H;   // 198
