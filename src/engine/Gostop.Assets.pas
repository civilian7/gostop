...
type
  ...
  public
    ...
    /// <summary>
    ///   AI 난이도 카드 전용 캐릭터 이미지 폴더(<c>assets\difficulty</c>)를 반환합니다. 못 찾으면 빈 문자열.
    ///   20인 아바타 로스터와 완전히 별개의 캐릭터라 avatars 하위가 아닌 assets 바로 아래에 둔다.
    /// </summary>
    /// <param name="ARoot">에셋 루트(<c>assets\hwatu</c>). 빈 문자열이면 <see cref="FindRoot"/>로 자동 탐색.</param>
    class function DifficultyDir(const ARoot: string = ''): string; static;
...
class function THwatuAssets.DifficultyDir(const ARoot: string): string;
begin
  var LRoot := ARoot;
  if LRoot = '' then
  begin
  ...
  // LRoot = <...>\assets\hwatu → 형제 폴더 <...>\assets\difficulty
  Result := TPath.Combine(TDirectory.GetParent(LRoot), 'difficulty');
end;
