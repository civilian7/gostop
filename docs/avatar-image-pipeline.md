...
"이미지 배경제거는 rembg" 항목. 시트 자체(20명 격자, 표정별)를 새로 생성할 때 쓴
실제 프롬프트는 `assets/gemini_prompt.txt` 참조 — 새 감정 상태를 추가할 때 같은
톤으로 프롬프트를 이어 쓰면 된다.
...
- 원본: `assets/avatars/raw/sheet_{normal,cheer,sad,angry}.png` — 각 2048×2048, 흰 배경
...
python scripts/avatar_bg_remove.py sheet assets/avatars/raw/sheet_angry.png assets/avatars/states "avatar_{:02d}_angry.png"
```
