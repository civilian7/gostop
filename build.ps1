...
# 3) 실행 리소스 동기화: 런타임에 필요한 것만 복사(아바타 png·효과음 wav). 화투패 이미지는
#    이제 Gostop.rc에 리소스(RCDATA)로 내장되어 exe 안에 있으므로 assets\hwatu는 통째로 제외.
#    svg·ogg·문서(md/tsv)·아바타 이미지 파이프라인 원본(raw 시트·구 소스시트·프롬프트 메모)도 제외
#    (avatars\raw·_source_sheet.png·gemini_prompt.txt는 에셋 재생성용 자료일 뿐 게임이 안 읽음)
robocopy "$Root\assets" "$Root\bin\assets" /MIR `
  /XD "$Root\assets\hwatu" "$Root\assets\avatars\raw" `
...
# robocopy /XF·/XD 는 제외 파일/폴더를 삭제하지 않으므로, 기존 배포분에 남은 불필요 파일/폴더 정리
